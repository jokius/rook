...
extension ControlServer {
    ...
    /// Apply a session's current watermark spec to its realized main + split + scratch surfaces. A
    /// never-realized surface (nil) is skipped — it applies the spec itself on creation
    /// (`GhosttySurfaceView.createSurface`).
    ...
    func readSessionText(_ target: String?, window: String?, options: ControlSessionTextOptions) -> ControlResponse {
        ...
        return resolver.resolveSession(target, window: window) { store, id in
            ...
            // while the display sleeps. Without this the same state answered `failed to read surface buffer`
            // here and `session not realized` from every other command, naming a cause that did not happen:
            // nothing failed to read, there was nothing to read from.
