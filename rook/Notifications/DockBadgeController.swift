...
///
/// **Uses the modern UserNotifications badge, NOT `NSApp.dockTile.badgeLabel`.** For rook the legacy
...
/// (it rides `.rookAppearanceChanged`, the same channel the sidebar uses).
@MainActor
final class DockBadgeController {
    ...
    /// `rookApp`; the library outlives the controller by app lifetime, matching `NotificationManager`.
    ...
    func start() {
        if appearanceObserver == nil {
            appearanceObserver = NotificationCenter.default.addObserver(
                forName: .rookAppearanceChanged, object: nil, queue: .main
            ) { [weak self] _ in
    ...
    /// re-arm here — they come through `start()`'s `.rookAppearanceChanged` observer instead.
    private func apply() {
