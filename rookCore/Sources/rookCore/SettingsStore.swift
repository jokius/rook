...
/// snapshot (so the `ROOK_STATE_DIR` test override applies). Mirrors `PersistenceStore`.
///
/// Recovery contract: a missing file or corrupt JSON resolves to the default settings with the
/// app's default theme seeded (`AppSettings.defaultTheme`) — a fresh install opens on the rook
