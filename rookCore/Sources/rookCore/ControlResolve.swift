...
public enum ControlResolve {
    ...
    /// Derive the control socket path. With `stateDir` (the `ROOK_STATE_DIR` value, if set) it is
    /// `<stateDir>/rook.sock`; otherwise `<appSupport>/rook.sock`. The app and the CLI both call this
    ...
    public static func socketPath(stateDir: String?, appSupport: String) -> String {
        ...
        return (base as NSString).appendingPathComponent("rook.sock")
    }
}
