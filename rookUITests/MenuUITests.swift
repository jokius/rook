...
final class MenuUITests: XCTestCase {
    ...
    // rook ships its OWN rebindable "Toggle Full Screen" (⌃⌘F) View item so full screen is drivable from
    ...
    // rook's item and NOT the native "Enter Full Screen".
    func testViewMenuHasSingleFullScreenItem() throws {
        XCTAssertTrue(app.staticTexts["session-row"].firstMatch.waitForExistence(timeout: 20), "seeded session should exist")
        app.menuBars.menuBarItems["View"].click()
        let toggle = app.menuItems["Toggle Full Screen"]
        XCTAssertTrue(toggle.waitForExistence(timeout: 5), "View menu should offer rook's Toggle Full Screen")
