...
public struct PaletteContext: Sendable, Equatable {
    ...
    /// (the tree's `workspaceFilter`). Named for the fact it reports: the marked set is what Clear Focus
    /// empties and what Toggle Workspace Filter has something to apply.
    ...
    /// to "Remove from Focus"). The ONLY term that entry keys on — marking is offered in either sidebar
    /// mode.
...
public enum PaletteCommand: String, CaseIterable, Sendable {
    ...
    public func isVisible(in context: PaletteContext) -> Bool {
        ...
        case .addWorkspaceToFocus:
            ...
            // membership is model state the tree applies the moment it is shown again, and every sibling is
            // mode-agnostic too — Focus Workspace, Toggle Workspace Filter, Clear Focus, the
            // `focus_workspace` keybind and `workspace.focus`/`workspace.filter`. The tree-mode gate
            // belongs to expand/collapse, which manipulate rows flagged mode never renders.
