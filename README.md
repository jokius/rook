...
Download the latest `.dmg` from the [releases page](https://github.com/jokius/rook/releases), open it, and drag `rook.app` into `/Applications`.
...
# the binary is at rookCore/.build/release/rookctl
```
...
rookctl session new --command "clear; ssh user@host"  # --command runs under a login shell, so operators, $VAR, redirects and your PATH all work
rookctl session new --shell /opt/homebrew/bin/fish  # spawn a specific shell (absolute path); defaults to your own $SHELL
...
**Shells and `--command`.** A session created over the socket runs *your* shell: `session new`, `window new`, and `quick` take `--shell <absolute path>`, and `rookctl` fills it from the caller's `$SHELL` by default — so a session you create from fish comes up fish, not whatever shell the app itself inherited. Only the shell path is inherited, never your environment (it carries the session and agent ids, and passing those on would make the new session report its agent status onto the old session's row); the login shell rebuilds the rest from its own rc files. `session duplicate`, `session split`, and `session scratch` take no `--shell` — they inherit from the session they belong to, since half a session in a different shell is exactly the surprise this avoids. A malformed value is rejected with `invalid shell (expected an absolute path)`, and a path that isn't an executable file with `shell not found or not executable: <path>`; the session's shell is persisted, comes back after a relaunch, and reads back as the `shell` field on the session's `tree` node (omitted when it uses the app default). `--command` runs under a login shell too (`<shell> -l -c '<cmd>'`), so your rc files run, `PATH` is yours — a bare Homebrew binary is found — and shell operators (`;`, `&&`, `|`, `$VAR`, redirects, globs) work as written, with no wrapper needed. Two consequences worth knowing: an rc file that does a `cd` overrides `--cwd`, and `session overlay open` is *not* part of this — an overlay still runs its command through `sh -c` with the app's `PATH` and no rc, so a bare Homebrew binary there still exits 127 and still wants an absolute path or a `zsh -lc '…'` wrapper.
...
rookctl session type --target work --select "$sel"  # paste it into another session
```
...
rookctl session copy --target 9f3c
```
...
rookctl session overlay close --target 9f3c                  # close it from a script
```
...
rookctl session status idle --target "$ROOK_SESSION_ID"        # clear it
```
...
To wire this up automatically, **Help ▸ Install Agent Status Hooks…** installs a hooks package. It copies the scripts to `~/.config/rook/agent-status/` (baking in the bundled `rookctl`'s path so the hooks work even without the CLI on your PATH), adds a `source` line to `~/.zshrc`, `~/.bashrc`, and `~/.config/fish/config.fish` for the generic shell integration, and merges four Claude Code hooks into `~/.claude/settings.json` (backing up the prior file as `.bak`, or leaving it untouched and skipping the merge if it isn't valid JSON): a prompt sets `active`, each tool that runs re-asserts `active` (so the status returns to active when work resumes after you answer a permission prompt), the Stop event sets `completed --auto-reset`, and a permission prompt sets `blocked`. It is idempotent — re-running refreshes the baked path, rewrites a rook hook whose arguments an older version installed, and is a clean no-op for entries already up to date. That rewrite matters: the merge used to recognize its own hook by the script path alone and skip it, so a change to what the hook is *called with* never reached anyone who already had it installed.

**Claude Code subagents move the row, and that is on purpose.** Claude Code fires those same hooks *inside* a subagent (the Task tool) and stamps them with the same session id, so a working subagent keeps the row on `active` just like the main thread does. Rook used to drop those reports (the payload's `agent_type` is absent on the main thread and carries the subagent's own type inside one), because they kept re-asserting `active` over the `completed` the main thread had already reported. That filter is gone: the `background_tasks` check below fixes the same lie more precisely, and the filter cost far more than it saved — while a swarm of agents is grinding, *their* hooks are the only signal that arrives at all, so the row fell silent exactly when the session was busiest. One known limitation comes with it: if the main thread is `blocked` waiting on you and a background agent is still working, the agent's next report can overwrite that `blocked` with `active`. Because an installed hook is a copy, re-run **Help ▸ Install Agent Status Hooks…** once to pick this up.

**And a finished turn with work still running does not say "done".** Claude Code's `Stop` event means the main thread ended its *turn*, not that the work is finished — in a swarm the lead hands the turn back while its backgrounded teammates keep grinding, and the `Stop → completed --auto-reset` hook lit the sidebar's done-checkmark and called you over for nothing. The `Stop` payload lists that work in `background_tasks`, so when at least one entry is still live the hook reports `active --blink` instead. Live means an entry whose `type` is `subagent` (a Task or a teammate) or `workflow` (an orchestrator) *and* whose `status` is not one of `completed`, `failed`, `cancelled`, `stopped` — a background shell (`bun run dev`, `tail -f`) is deliberately not live, since it sits there for hours and would hold the row forever. Nothing has to re-light the checkmark: Claude Code wakes the main thread when the last agent lands, and its next `Stop` carries an empty list. The trade-off is deliberate — if the lead ends its turn to ask *you* something while teammates are still running, the row shows `active` rather than the checkmark. Today it lies the other way, calling you when it is far too early, and being pulled over for nothing costs more than a question noticed a minute late.

**An agent your agent started still does not move the row.** A nested agent *process* — a `claude -p …` the pane's agent spawns to do a side job — is the main thread of its own session, so its hook payload is indistinguishable from your agent's; it used to report `completed` when *it* finished, and the row said "done" while your real agent kept working. Rook applies to `session status` the same ownership test `session agent` already used: `rookctl` reports its nearest agent *ancestor*, and a status is dropped when that pid differs from the pane's foreground process (the call still succeeds, it simply changes nothing). This is a different case from the `background_tasks` check above and you need both — a nested process reports from its own pid, an in-process subagent shares the pane's.
...
The same installer also drops in `rook-agent-session.sh` and wires it to the `SessionStart` event of both Claude Code and Codex. That hook is what makes [resuming agent conversations](#resuming-agent-conversations) work: it reports the conversation the pane is on, which nothing else can know. If you installed the hooks before this feature landed, re-run **Help ▸ Install Agent Status Hooks…** to pick it up (it is idempotent).

Installing copies the scripts, so an installed copy never updates itself. That matters for the `--pane-id` forwarding above: hooks installed before it landed keep reporting the pane by its baked role alone, which is the mis-attribution case described there. Re-run **Help ▸ Install Agent Status Hooks…** once to refresh the copy, then start a fresh shell. (The hook *entries* in `~/.claude/settings.json` are a different story — as of the rewrite described above, re-running does update those in place.)

**Quitting gives a working agent a moment to wrap up.** Closing Rook kills every session's shell outright, and an agent mid-task loses whatever it hadn't written down — no signal reaches it, because the pty's session leader is the surviving `login`, so not even a SIGHUP arrives. So on quit, if any session has an agent mid-turn, Rook drops a one-line flag next to its control socket, and the already-installed `PostToolUse` hook — the one thing a busy agent runs between every tool call — reads it and hands the agent a blocking message: the terminal is closing, wrap up. Rook waits up to **Settings ▸ Agent Status ▸ Let agents wrap up before quitting** (15s by default, 0 turns it off), and closes as soon as every agent has finished rather than sitting out the whole budget — so the number is a ceiling, not a pause you pay on every quit. What the agent does with the warning is its own call — write a short handoff note, or just stop cleanly. A system logout skips the quit confirmation entirely and drains silently, so Rook no longer parks your logout on a dialog.

Be clear-eyed about what it cannot reach. The warning rides the agent's own hook, so it arrives only *between* tool calls: an agent sitting inside one long call — a ten-minute build — never sees it, and neither does a session with no hooks installed, nor Codex or Pi (their adapters don't carry the flag yet). A forced power-off or a `kill -9` runs none of this at all. It covers an agent working in a loop of tool calls, which is the common case, not every agent everywhere.
...
rookctl session agent claude --clear              # forget the pane's conversation
```
...
Limitations, on top of the [restore limitations](#restore-limitations) the feature inherits: it only works after a clean quit (a force-quit captures nothing); without the `SessionStart` hook installed there is no conversation id, so the pane falls back to `claude --continue` / `codex resume --last` — the last conversation in that directory, which means two panes in the same directory both land on the same one; and an agent running behind `tmux` or over `ssh` is not restored at all.

## Troubleshooting

Where the logs and config live, how to read them, and the common problems (a keymap editor that will not open, a custom action that does nothing, missing notifications) are covered in [docs/troubleshooting.md](docs/troubleshooting.md). For a bug, open an [issue](https://github.com/jokius/rook/issues/new); for a feature request or question, start a [Discussion](https://github.com/jokius/rook/discussions/new).

## Restore limitations

Restore reconstructs the structure, not the running processes. Three limitations follow from the design:

1. Live processes are not reattached — true process survival would require a tmux-style backend, which is out of scope. By default a restored session re-spawns a fresh login shell in its saved working directory. The optional **Restore running commands on restart** toggle (General settings, off by default) re-runs the command each pane had in the foreground at the last clean quit, so a gate `ssh`, `tail -f`, or `top` comes back — but it is a re-run, not a reattach: only a single-process command restores faithfully (pipelines and compound lines do not); a force-quit or crash captures nothing, and a capture replays exactly once — the launch that arms it clears it from the state file, so a crash later in that session can cost you the restore but never re-runs a command twice; and the programs named in `restore-denylist.conf` (in the config directory, seeded with the terminal multiplexers `tmux`/`screen`/`zellij`, one command name per line) are skipped so they start fresh rather than re-launching — everything else, including `python manage.py runserver` or `node server.js`, is restored. Edit that file to add or remove entries. A coding agent re-run this way comes back on a blank conversation unless you also turn on **Resume agent conversations** — see [Resuming agent conversations](#resuming-agent-conversations).
