...
- **Releases are cut LOCALLY, not by CI — there is NO `release.yml`.**
  ...
  `Developer ID Application: Brave Elk LLC` cert and the `rook-notary` keychain profile live.
  ...
  cask to `jokius/homebrew-apps` with the maintainer's own `gh` auth (no `HOMEBREW_TAP_PAT` needed).
