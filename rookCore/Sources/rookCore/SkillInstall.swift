...
/// Pure, host-free helpers for installing the bundled agent skill (`rook`) into the skills
...
public enum SkillInstall {
    ...
    public static let skillName = "rook"

    /// A sentinel embedded in the bundled `SKILL.md` (an HTML comment, invisible when rendered) so a
    /// reinstall can tell an rook-authored skill from a same-named skill the user wrote themselves,
    /// and refuse to clobber the latter.
    public static let marker = "<!-- rook-skill -->"

    /// One install destination: which agent, and the `…/skills/rook` directory to write.
    public struct Target: Equatable, Sendable {
        public let agent: String          // human label, e.g. "Claude Code"
        public let skillDirectory: String // the install path, e.g. <home>/.claude/skills/rook
    ...
    /// `.claude` or `.codex`), so the result is `<home>/<base>/skills/rook`.
    ...
    /// `SKILL.md` carries `marker` (i.e. rook put it there) — a present directory with an absent,
    /// unreadable, or unmarked `SKILL.md` is treated as user-authored content and refused, so a user's
    /// own skill named `rook` is preserved rather than recursively wiped.
