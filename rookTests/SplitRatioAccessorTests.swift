...
/// The split divider's cursor writer and its double-click reset, neither of which anything else covers:
/// `NSCursor` is process-global and the reset is recognized from a raw mouse event, so both are only
/// observable by driving a real `NSSplitView`. Also pins the tracking-area handback — `NSTrackingArea` does
/// not retain its owner, so a probe that leaves the window without handing its area back leaves the split
/// messaging a freed view on the next mouse move.
@MainActor
final class SplitRatioAccessorTests: XCTestCase {
    ...
    /// The tracking area covers the whole split, not just the band, so every move over a pane arrives here too.
    ...
    // MARK: - double-click restores the even split
