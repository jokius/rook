...
// terminal-output knowledge belongs to this installed hook, not to rook's runtime status engine.
struct CodexStatusHookTests {
    private static var hook: String {
        URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .appendingPathComponent("rook/Resources/agent-status/rook-codex-status.sh")
            .path
    }
    ...
    private func run(_ action: String, screen: String = "", screens: [String] = [], worker: Bool = false,
        ...
        let dir = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("rook-codex-hook-\(UUID().uuidString)")
        ...
        let rookctl = dir.appendingPathComponent("rookctl")
        ...
        """.write(to: rookctl, atomically: true, encoding: .utf8)
        try fm.setAttributes([.posixPermissions: 0o755], ofItemAtPath: statusWrapper.path)
        try fm.setAttributes([.posixPermissions: 0o755], ofItemAtPath: rookctl.path)

        let proc = Process()
        proc.executableURL = URL(fileURLWithPath: "/bin/bash")
        proc.arguments = worker ? [Self.hook, "__watch-blocked", "token", tokenFile.path] : [Self.hook, action]
        var environment = [
            "ROOKCTL": rookctl.path,
            "ROOK_STATUS_WRAPPER": statusWrapper.path,
            "ROOK_SESSION_ID": "sid",
            "ROOK_SOCKET": "/tmp/rook.sock",
            "ROOK_PANE": "right",
            "ROOK_CODEX_WATCH_FILE": tokenFile.path,
            "ROOK_CODEX_WATCH_MAX_CHECKS": worker ? String(frames.count) : "0",
            "ROOK_CODEX_WATCH_INTERVAL": "0",
            "PATH": "/usr/bin:/bin",
        ]
        if supersedeTokenOnRead { environment["SUPERSEDE_ON_READ"] = "1" }
        proc.environment = environment
        // the `stop` arm reads the hook payload from stdin, so hand it an explicit pipe and close it:
        // an inherited stdin that never reaches EOF (a terminal) hangs `plutil -extract … -` forever.
    ...
    @Test func hookIsSilentNoOpOutsideRook() throws {
        let proc = Process()
        proc.executableURL = URL(fileURLWithPath: "/bin/bash")
    ...
    @Test func watcherIgnoresAutoReviewProgress() throws {
        ...
        #expect(result.controlCalls == ["session text --target sid --socket /tmp/rook.sock --pane right"])
        #expect(result.statusCalls.isEmpty)
    }
