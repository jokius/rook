...
struct rookApp: App {
    ...
    init() {
        ...
        // settings persist alongside the workspace snapshot (same ROOK_STATE_DIR override). Built
        ...
        let settingsStore = ProcessInfo.processInfo.environment["ROOK_STATE_DIR"]
        ...
        // sleeps, which leaves a scheduled job's session realized-never and its --command unrun.
        ...
        WindowGroup(id: Self.windowGroupID) {
            ContentView(
                ...
                .task {
                    ...
                    // the keymap on `.rookKeymapChanged`. Hand the delegate a reference so it can
                    ...
                    // hand the delegate the action hub and drain any folders an `open -a rook /path`
    ...
    /// window set is always valid and non-empty. UI tests pass `ROOK_STATE_DIR` to isolate persistence
    ...
    private func surfaceEnv(for session: Session, pane: StatusPane? = nil) -> [String: String] {
        ...
        // a session-owned pane (main/split/scratch) gets a fresh stable token baked as ROOK_PANE_ID so the
        // agent-status hook can forward it as --pane-id and the status handler resolves the surface's LIVE
        // slot rather than the baked role; the overlay (pane == nil) needs none.
