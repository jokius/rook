...
extension WorkspaceSidebar.Coordinator {
    ...
    /// The inline "+" button for a workspace row: a template `plus` glyph tinted like the row icon
    /// (see `SidebarCellView.setColors`), hidden until the row is hovered. It carries no workspace id —
    /// `addSessionButtonClicked` derives the workspace from the outline row at click time, so a reused
    /// cell always targets the right workspace.
