...
/// App-side host for the per-window native picker (`pick.open`/`pick.result`/`pick.cancel`). Argument
/// validation, error strings, and response shape stay in `ControlDispatcher+Pick`; this layer only
/// resolves the target window and drives its registered `PickController`. Its own file rather than
/// `ControlServer+SessionActions.swift`, which is at the swiftlint size limit; the `ControlActions`
/// conformance stays declared there and an extension in any file of the module satisfies it.
