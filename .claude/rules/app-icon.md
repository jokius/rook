...
- The icon is an **adaptive Icon Composer document** at `rook/AppIcon.icon` (macOS 26 `.icon` format
...
- **The `.icon` MUST be a target FILE, NOT nested inside `Assets.xcassets` (load-bearing).** actool only
  ...
  icon-less).
  So `AppIcon.icon` lives at the top of the `rook/` source dir (Xcode types it `wrapper.icon` and routes
  it to actool alongside the catalog: `actool …/AppIcon.icon …/Assets.xcassets --compile …`).
  To swap the design, replace `rook/AppIcon.icon` (re-export from Icon Composer;
...
- **xcodegen also lists the `.icon` in Copy Bundle Resources**, dropping a redundant LOOSE `AppIcon.icon`
  in the bundle.
  The `Bundle rookctl CLI` postBuildScript `rm -rf`s it before the re-seal:
