...
// Tests the shipped panel painter `rook/Resources/hud/hud.sh` by running it against a temp body file and
// reading the escape stream it writes. It reaches the app target's resource on purpose: the script is the
// rendering half of the rookCore `HudLayout` model, and the three contracts (the header line, the
...
struct HudHelperTests {
    private static var helper: String {
        URL(fileURLWithPath: #filePath)      // …/rookCore/Tests/rookCoreTests/HudHelperTests.swift
            .deletingLastPathComponent()     // rookCoreTests
            .deletingLastPathComponent()     // Tests
            .deletingLastPathComponent()     // rookCore
            .deletingLastPathComponent()     // repo root
            .appendingPathComponent("rook/Resources/hud/hud.sh")
            .path
    }
    ...
    private final class Run {
        ...
        init(_ body: String, cols: Int, rows: Int, spinner: HudSpinner? = nil,
            ...
            dir = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("rook-hud-\(UUID().uuidString)")
    ...
    @Test func theShippedHelperIsWhereTheseTestsLookForIt() throws {
        #expect(FileManager.default.isReadableFile(atPath: Self.helper),
                "no helper at \(Self.helper): rook/Resources/hud/hud.sh moved, or this file did")
    }
