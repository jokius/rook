...
public struct RecentClosedWorkspace: Codable, Equatable, Sendable {
    ...
    /// the closed workspace, and one store is shared by every window (see `AppStore.markFocusMember`).
    /// OPTIONAL because this struct is persisted in `recent-closed.json`: a required key would fail the
    /// whole decode on a file written before it existed, and `RecentClosedStore.load()` turns a decode
    /// failure into an EMPTY list — the user's entire recent list, wiped on the first launch after the
    /// upgrade. Absent (nil) reads as "not a member", the behavior of every pre-existing entry.
...
public struct RecentClosedStore: Sendable {
    ...
    private func save(_ items: [RecentClosedItem]) {
        do {
            ...
            NSLog("rook: save recent closed failed: %@", String(describing: error))
        }
    }
}
