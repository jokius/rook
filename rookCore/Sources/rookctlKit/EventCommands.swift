...
/// A failure that stops the poll loop: a transport error, a server `ok:false` (notably a loud cursor
/// failure), or an output write that failed. `rookctl events` never swallows one — a consumer that lost
/// its place must find out, not keep printing as if nothing happened.
struct EventStreamError: Error, CustomStringConvertible {
    let description: String
    init(_ description: String) { self.description = description }
}

/// The three effects the poll loop performs, injected so the loop itself is testable without a socket,
/// a real clock, or stdout.
...
/// The client's position in the stream. The first request carries no cursor (subscribe-from-now); every
/// reply's `next` becomes the next request's `--after`, so no event is read twice and none is skipped.
...
/// One line per event, either bare JSON (NDJSON — pipe it into `jq`) or a human column layout.
enum EventFormatter {
...
/// `rookctl events` — a CLIENT-side poll loop over the one-shot `events.read` command. Deliberately not
/// a push/streaming socket: the server keeps its one-request-one-connection model, and this just asks
/// again. The 0.25 s backoff runs ONLY after an empty page, so a burst drains at full speed.
