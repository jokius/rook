...
final class SidebarRenameController: NSObject, NSTextFieldDelegate {
    ...
    func beginEditing(node: SidebarNode) {
        ...
        // custom name (`renameOriginalValue`, captured above, still restores the decorated label on cancel).
        // tree mode already shows the bare name, so this is a no-op there.
