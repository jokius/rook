...
private let logger = Logger(subsystem: "com.rook.app", category: "GhosttySurfaceView")
...
final class GhosttySurfaceView: NSView, TerminalSurface {
    ...
    /// Extra environment variables (the `ROOK_*` vars) the spawned shell sees, set into the surface
    ...
    /// `--command` unrun until something incidental re-lays it out. `object: nil` and a token in
    /// `focusObservers` match the sibling observer, including its `NotificationCenter.default` teardown.
    private func observeDisplayWake() {
        let token = NotificationCenter.default.addObserver(
            forName: .rookScreensDidWake, object: nil, queue: .main
        ) { [weak self] _ in
        ...
        for name in [NSWindow.didBecomeKeyNotification, NSWindow.didResignKeyNotification] {
            ...
            let token = center.addObserver(forName: name, object: nil, queue: .main) { [weak self] _ in
                MainActor.assumeIsolated {
                    ...
                    // returning focus to rook (cmd-tab or a reactivating click) while this pane is the
                    ...
                    // first responder never resigned while rook was backgrounded, so no focus transition
    ...
    /// rook regains key focus on it — the inverse of notification suppression (which drops a banner only
    ...
    func createSurface() {
        ...
        // extra environment for the spawned shell (the ROOK_* vars). Each key/value is strdup'd into
        ...
        guard let surface else {
            ...
            // the report undiagnosable from outside: `session.new` had already answered ok. Re-arm the
            // deferred-create flag so the layout path retries too — `.rookScreensDidWake` is what makes
            // recovery TIMELY, not what makes it possible, and a view first mounted inside the residual
            // post-wake window registers its observer too late for the wake that just fired.
            pendingSurfaceCreation = true
