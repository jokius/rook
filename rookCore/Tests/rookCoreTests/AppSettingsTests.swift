...
struct AppSettingsTests {
    ...
    @Test func followingEmitsRawDual() {
        ...
        // resolves the active side itself on a color-scheme change, so rook never picks a side.
    ...
    @Test func activeThemeTracksAppearanceWhenFollowing() {
        ...
        let single = AppSettings(theme: "rook")
        #expect(single.activeTheme(isDark: true) == "rook")
        #expect(single.activeTheme(isDark: false) == "rook")
    ...
    @Test func newThemeFieldsRoundTrip() throws {
        let original = AppSettings(theme: "Builtin Light", darkTheme: "rook", followSystemAppearance: true)
        let decoded = try JSONDecoder().decode(AppSettings.self, from: JSONEncoder().encode(original))
        #expect(decoded == original)
        #expect(decoded.darkTheme == "rook")
    ...
    @Test func configDirectoryRoundTripsAndIsNotAConfigLine() throws {
        let original = AppSettings(configDirectory: "/tmp/rook-config")
        let decoded = try JSONDecoder().decode(AppSettings.self, from: JSONEncoder().encode(original))
        #expect(decoded.configDirectory == "/tmp/rook-config")
    ...
    @Test func defaultThemeIsRookButNotBakedIntoAppSettings() {
        ...
        #expect(AppSettings(theme: AppSettings.defaultTheme).ghosttyConfigLines().contains("theme = rook"))
    }
