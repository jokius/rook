...
private let logger = Logger(subsystem: "com.rook.app", category: "WatermarkRenderer")
...
/// default text tint), so it lives in the app target, not the host-free `rookCore`.
@MainActor
enum WatermarkRenderer {
    ...
    static func materialize(_ watermark: BackgroundWatermark?, sessionID: UUID) -> String? {
        ...
        case .text:
            ...
            let color = NSColor(rookHex: watermark.colorHex) ?? GhosttyApp.shared.terminalForegroundColor ?? .white
