...
struct Session: ParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        subcommands: [New.self, Duplicate.self, Close.self, Select.self, Go.self, Rename.self, Reveal.self, Move.self, TypeText.self,
                      ...
                      SelectAll.self, Text.self, Status.self, Restore.self, Agent.self, FlagCommand.self,
    ...
    struct Split: ParsableCommand {
        ...
        /// `rookctl session split [on|off|toggle]` — the default subcommand, so the bare verb keeps working
        /// (the `quick` group's shape).
    ...
    struct Status: RequestCommand {
        ...
        var color: String?
        @Option(name: .long, help: """
            Override the glyph silhouette for this call only: \(StatusShape.validNamesPhrase); \
            reverts on the next status set without it. Omitted keeps the state's own glyph \
            (ellipsis / exclamation mark / check mark).
            """)
        var shape: String?
        ...
        @Option(name: .customLong("pane-id"), help: """
            A surface's stable token (the shell's $ROOK_PANE_ID) — the agent-status hook forwards it so a \
    ...
    struct Restore: RequestCommand {
        static let configuration = CommandConfiguration(
            ...
            discussion: """
            session restore "cd api && npm run dev"   pin a shell line for the next launch
            ...
            session. It wins over the pane's captured foreground command and over the session's own \
            --command, is typed VERBATIM (so a pipeline or a compound line restores, which the argv \
            capture cannot do), skips restore-denylist.conf (you named this command deliberately), and \
            reads back on `tree` as restoreCommand (main pane) or splitRestoreCommand (split pane). It is \
            STICKY: it fires again on every launch until cleared.

            It still obeys Settings > General > "Restore running commands on restart" — with that off \
            nothing is restored, pin included.

            COMMAND is shell code, stored verbatim in the window's state file and readable via `tree`, so \
            it must not carry secrets.

            Not to be confused with `rookctl restore clear`, which is app-global and clears every \
        ...
        @Option(name: .long, help: "Which pane to pin: left (main) or right (split); scratch is rejected — the scratch terminal is never restored. Defaults to the left pane.") var pane: String?
        @Option(name: .customLong("pane-id"), help: """
            A surface's stable token (the shell's $ROOK_PANE_ID) — resolves to the pane's CURRENT slot, so \
            a hook in a promoted-then-re-split pane still pins the right one. Unlike `session status`, a \
            token that does not resolve is an error unless --pane is also given as the fallback.
            """)
        var paneID: String?
        @OptionGroup var target: TargetOptions
        @OptionGroup var options: ClientOptions
