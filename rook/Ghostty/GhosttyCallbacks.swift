...
private let logger = Logger(subsystem: "com.rook.app", category: "GhosttyCallbacks")
...
final class GhosttyCallbacks: @unchecked Sendable {
    ...
    /// 120Hz poll timer — rook is demand-driven now.)
    ...
    func action(target: ghostty_target_s, action: ghostty_action_s) -> Bool {
        ...
        case GHOSTTY_ACTION_RENDER:
            // libghostty signals this surface has a frame ready to paint. rook is demand-driven (no poll
    ...
    /// user may paste the same path again later), so rook cannot know when it is done — macOS's own temp
    ...
    private static func pasteboardImagePath(_ pb: NSPasteboard) -> String? {
        ...
        let file = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("rook-paste-\(UUID().uuidString).png")
        do {
