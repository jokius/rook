...
/// machine: Claude Code (`~/.claude/skills/rook/`) and Codex (`~/.codex/skills/rook/`) — both use
/// the same SKILL.md Agent-Skill format. So an agent running INSIDE an rook session learns how to
/// drive the app via `rookctl`. The host-free path/target/ownership logic lives in
/// `rookCore.SkillInstall`; this type owns the AppKit filesystem glue. Idempotent: re-running
...
enum SkillInstaller {
    ...
    private static func install() throws -> Report {
        ...
        for target in targets {
            ...
            // SKILL.md is absent, unreadable, or lacks the rook marker). `attributesOfItem` uses lstat
            ...
            do {
                ...
                try? fm.removeItem(at: destination) // drop a prior rook install (ignore if absent)
    ...
    private static func message(_ report: Report) -> String {
        ...
        if report.installed.isEmpty {
            ...
            lines.append("")
            lines.append("The agent uses it automatically when you ask it to control rook. It drives the "
                + "app via the rookctl CLI, so install that too if you haven't: Help ▸ Install Command Line Tool…")
        }
