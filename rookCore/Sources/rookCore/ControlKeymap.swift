...
public struct ControlKeymapMenuItem: Codable, Sendable, Equatable {
    ...
    public var chord: String
    /// The item's Objective-C action selector. rook's own SwiftUI items all report `menuAction:`;
    /// anything else is an AppKit-supplied item (`performClose:`, `closeAll:`, …), which is what a
    /// stock item competing for a rook chord looks like.
...
public extension ControlKeymap {
    ...
    /// way the enum's own declaration does (window, workspace, session, …) instead of scattering related
