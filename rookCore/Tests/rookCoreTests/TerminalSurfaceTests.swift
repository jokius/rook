...
struct TerminalSurfaceTests {
    // rook's natural direction is INVERTED to libghostty's navigate_search string: libghostty's `next`
    // walks newest→oldest (visually UP) and `previous` walks oldest→newest (visually DOWN), so rook's
