...
struct TerminalView: NSViewRepresentable {
    ...
    /// (each terminal host carries stable session + surface identity, so a session becoming active — or the
    /// hosted surface being replaced — yields a fresh representable). On later updates focus is left
