...
struct AppStorePaneTests {
    ...
    // `session.new` answers ok for a model insert, and libghostty refuses to build a surface while the
    // display sleeps, so this is the field that separates a working session from an empty one.
    @Test func controlTreeReportsMainPaneRealization() {
        ...
        let parked = SpySurface(isRealized: false)
    ...
    @Test func setAgentIndicatorCoercesRightToLeftWithoutLiveSplit() {
        // a promoted survivor's shell keeps its baked `ROOK_PANE=right`, so the agent-status hook re-emits
    ...
    @Test func setAgentIndicatorKeepsRightDuringSplitRealization() {
        ...
        // a `splitSurface == nil` gate would rewrite this tag and fail the test.
        let store = makeStore()
        let ws = store.addWorkspace(name: "work")
        let session = store.addSession(toWorkspace: ws.id, cwd: "/a")!
        session.surface = SpySurface()
