...
private let logger = Logger(subsystem: "com.rook.app", category: "CustomCommandRunner")
...
///
/// `@MainActor` and constructed once as `@State` in `rookApp`. `start()`/`stop()` install/remove
...
/// on the `.rookKeymapChanged` notification. Pure parsing/matching/expansion lives in rookCore;
/// this class only maps `NSEvent` → rookCore types, owns the leader timeout timer, resolves the
...
final class CustomCommandRunner {
    ...
    /// keymap, and observe `.rookKeymapChanged` to rebuild on a keymap reload.
    func start() {
        ...
        keymapObserver = NotificationCenter.default.addObserver(
            forName: .rookKeymapChanged, object: nil, queue: .main
        ) { [weak self] _ in
    ...
    /// when the key window is a rook terminal window whose focus is NOT on a text field — including
    ...
    private func handleKeyDown(_ event: NSEvent) -> Bool {
        ...
        // with no focused terminal surface, fire ONLY from a rook terminal window (an empty one still
    ...
    /// active session's scratch and otherwise takes the emptied-window/palette fallback.
    ...
    /// are not panes (their state is queryable via `tree`), so they take the plain fallback (the active
    /// session, else the session-free launcher path — an emptied window).
    ...
    /// any surface (a quick terminal / overlay with no owning session). Uses the active session's context
    /// when one exists (like the palette), else the session-free launcher path via `spawnSessionless`.
    ...
    /// `AGT_SOCKET`/`AGT_WINDOW`/`AGT_PANE`, e.g. `rookctl session new --command "ssh …"`) still fires.
    ...
    /// chord reach `rookctl` to create a fresh session.
    private func sessionlessContext() -> CommandContext {
        let windowID = library.activeWindowID
        let windowName = windowID.flatMap { id in library.windows.first { $0.id == id }?.name } ?? ""
        return CommandContext(windowID: windowID?.uuidString ?? "", windowName: windowName,
                              socket: socketProvider())
    }
