...
/// Installs the bundled `rookctl` CLI into the user's PATH by symlinking it from the app bundle
...
/// `rookCore.CLIInstall`; this type owns the AppKit filesystem + authorization glue.
@MainActor
enum CLIInstaller {
    enum InstallResult {
    ...
    /// The bundled helper at `Contents/MacOS/rookctl`, or nil when this build skipped the bundling
    ...
    static func run() {
        ...
        case .installed(let path):
            present(style: .informational, title: "Command Line Tool Installed",
                    text: "rookctl was linked into \(path). Open a new terminal and run “rookctl --help”.")
