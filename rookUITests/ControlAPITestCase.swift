...
/// with an isolated `ROOK_STATE_DIR` (which also locates the unix socket at `<stateDir>/rook.sock`),
...
class ControlAPITestCase: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        stateDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("rook-ctluitest-\(UUID().uuidString)", isDirectory: true)
        markerDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("rook-ctlmarker-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: markerDir, withIntermediateDirectories: true)
        // socket path constraints: it must be (a) under the unix-socket sun_path ~104-byte limit and
        // (b) inside the runner's sandbox grant. The per-test ROOK_STATE_DIR subdir pushes the path to
        ...
        socketPath = (NSTemporaryDirectory() as NSString)
            .appendingPathComponent("rookc-\(UUID().uuidString.prefix(8)).sock")
        app = XCUIApplication()
        ...
        app.launchEnvironment["ROOK_UITEST_DOUBLECLICK_ACTION"] =
    ...
    /// file directly under the state dir (the `ROOK_STATE_DIR` override), and the app applies GUI-only
