...
extension XCUIApplication {
    ...
    /// FB11763863), then launches and brings the app to the foreground. Caller sets `ROOK_STATE_DIR`
    /// (and any test-specific env) before calling.
    func launchForUITest(file: StaticString = #filePath, line: UInt = #line) {
        launchEnvironment["ROOK_UITEST_FORCE_SIDEBAR_VISIBLE"] = "1"
...
extension URL {
    /// The persisted single-window snapshot file under an isolated `ROOK_STATE_DIR`. Per-window state
