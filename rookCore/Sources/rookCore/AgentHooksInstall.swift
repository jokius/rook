...
public enum AgentHooksInstall {
    ...
    public static let wrapperName = "rook-agent-status.sh"
    ...
    public static let piExtensionRelativePath = "pi/rook-status.ts"

    /// The Pi extension's destination filename under `~/.pi/agent/extensions/`.
    public static let piExtensionName = "rook-status.ts"
    ...
    public static let piExtensionMarker = "// rook-pi-status-extension"
    ...
    /// Marker lines bracketing the rook-managed block in a shell rc file. The opening marker is also
    /// the idempotency probe (present → already installed).
    public static let rcMarkerBegin = "# >>> rook agent-status >>>"
    public static let rcMarkerEnd = "# <<< rook agent-status <<<"
    ...
    /// The destination path for rook's Pi status extension.
    ...
    /// existing file must carry the rook ownership marker. An unreadable file is treated as user-owned.
    ...
    public enum CodexMergeOutcome: Equatable {
        ...
        /// The file already carries the current rook hooks block — nothing to do.
        case unchanged
    ...
    /// hook entry. e.g. `<scriptDir>/rook-agent-status.sh`.
