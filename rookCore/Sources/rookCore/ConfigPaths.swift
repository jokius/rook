...
public enum ConfigPaths {
    ...
    /// - else `<stateDir>/config` when `stateDir` (the `ROOK_STATE_DIR` value) is set — test isolation.
    /// - else `<home>/.config/rook`.
    public static func configDirectory(setting: String?, stateDir: String?, home: URL) -> URL {
        ...
        return home.appendingPathComponent(".config").appendingPathComponent("rook")
    }
    ...
    public static func starterKeymapConf() -> String {
        ...
        return """
        # rook keymap — a kitty-flavored config for rebinding built-in shortcuts and defining
        # custom shell commands. Edit this file and run File ▸ Reload Keymap (or `rookctl keymap
    ...
    /// The rook-scoped ghostty config file path within a resolved config directory: `<dir>/ghostty.conf`.
    ...
    public static func editorCommand(forPath path: String) -> String {
        ...
        let viaPosix = "exec /bin/sh -c \(singleQuoted(inner)) rook-config-edit \(singleQuoted(path))"
