...
/// Title-bar "jump to a session" buttons, split out of `WindowContentView+Titlebar` to keep that file
/// small (the `+Dashboard`/`+Zoom` extension pattern): the recent-sessions popover — the mouse equivalent
/// of the Ctrl-Tab switcher — and the attention bell. Both open a session picker over the frontmost window
/// and are referenced from `WindowContentView.titlebarTrailingActions`.
extension WindowContentView {
    ...
    /// The recent-sessions popover body: the MRU OTHER sessions as full-row `SessionPopoverRow`s (the shared
    ...
    /// Tinted to the terminal theme (`terminalColor` panel, `chromeText` text, brand selection-hover) so it
    ...
    /// The hover-highlight color for a popover row: rook's brand selection pill (`rookGreen`) at half opacity
    /// — the SAME color the selected sidebar row draws (see `WindowContentView.dashboardPillColor`). rook
    /// draws its own brand selection rather than the theme's `selection-background` (which `ghostty_config_get`
    /// can't return anyway; see libghostty.md), so unlike upstream there's no theme-selection branch.
    private var recentSelectionColor: Color {
        Color(nsColor: .rookGreen).opacity(0.5)
    }
    ...
    /// attention popover (the mouse form; ⌃⇧I / Navigate ▸ Go to Attention… keep the searchable `.attention`
    /// palette). Reading `store.attentionSessions` in the body registers the per-session `agentIndicator`
    /// observation, so the glyph re-renders live as a status changes. `.accessibilityValue`
