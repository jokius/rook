    ...
    var body: some View {
        Form {
            ...
            Section("Ghostty Config") {
                ...
                SettingHint("Also loads ~/.config/ghostty/config on top of rook's own. Edit ~/.config/rook/ghostty.conf to customize.")
            }
        }
        .formStyle(.grouped)
        .padding()
    ...
    var body: some View {
        Form {
            ...
            Section("Window") {
                ...
                // the sliders keep working and stay saved under Reduce Transparency — the window just
                // presents opaque until the system setting goes off, so say that instead of the
                // now-misleading blur dependency.
...
private struct AgentStatusSettingsView: View {
    ...
    private var activeStatusColor: Binding<Color> {
        Binding(get: { Color(nsColor: NSColor(rookHex: model.settings.activeStatusColorHex) ?? GhosttyApp.defaultActiveStatusColor) },
                set: { model.setActiveStatusColorHex(NSColor($0).rookHexString) })
    }

    private var blockedStatusColor: Binding<Color> {
        Binding(get: { Color(nsColor: NSColor(rookHex: model.settings.blockedStatusColorHex) ?? .systemOrange) },
...
private struct KeyMappingSettingsView: View {
    ...
    /// default location (`ROOK_STATE_DIR/config` under test isolation, else `~/.config/rook`),
