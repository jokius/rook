...
struct AppStoreCurrentWorkspaceTests {
    ...
    // a foreground create used to leave the previous workspace current, so Rename Workspace edited the
    // wrong row and the next new session landed in the old workspace.
    ...
    @Test func selectingASessionDropsTheFreshWorkspaceAsCurrent() {
        let store = makeStore()
        let work = store.addWorkspace(name: "work")
        let first = store.addSession(toWorkspace: work.id, cwd: "/a")!
        let other = store.addWorkspace(name: "other")
        store.addSession(toWorkspace: other.id, cwd: "/b")
    ...
    @Test func reselectingTheActiveSessionKeepsTheFreshWorkspaceCurrent() {
        let store = makeStore()
        let work = store.addWorkspace(name: "work")
        let session = store.addSession(toWorkspace: work.id, cwd: "/a")!
        let fresh = store.addWorkspace(name: "fresh")
    ...
    @Test func removingAnotherWorkspaceKeepsTheFreshTarget() {
        let store = makeStore()
        let work = store.addWorkspace(name: "work")
        let session = store.addSession(toWorkspace: work.id, cwd: "/a")!
        store.selectSession(session.id)
    ...
    @Test func closingTheSelectedSessionDropsTheFreshWorkspaceAsCurrent() {
        ...
        let work = store.addWorkspace(name: "work")
        let first = store.addSession(toWorkspace: work.id, cwd: "/a")!
        store.addSession(toWorkspace: work.id, cwd: "/b")
        store.selectSession(first.id)
        let fresh = store.addWorkspace(name: "fresh")
        #expect(store.currentWorkspaceID == fresh.id)
    ...
    @Test func closingTheOnlySessionDropsTheFreshWorkspaceAsCurrent() {
        let store = makeStore()
        let work = store.addWorkspace(name: "work")
        let session = store.addSession(toWorkspace: work.id, cwd: "/a")!
        store.selectSession(session.id)
    ...
    @Test func reopeningAClosedSessionDropsTheFreshWorkspaceAsCurrent() {
        ...
        store.restoreRecentClosed(recentClosed.load().first!)
        #expect(store.currentWorkspaceID == work.id)
    }
    ...
    @Test func reopeningAHardRemovedFreshWorkspaceDoesNotReviveTheTarget() {
        let (store, recentClosed, _) = makeStoreWithRecentClosed()
        let work = store.addWorkspace(name: "work")
        let session = store.addSession(toWorkspace: work.id, cwd: "/a")!
        store.selectSession(session.id)
        ...
        store.removeWorkspace(fresh.id)
        #expect(store.restoreRecentClosed(recentClosed.load().first!))
        #expect(store.workspaces.map(\.id) == [work.id, fresh.id])
        #expect(store.selectedSessionID == session.id)
        #expect(store.currentWorkspaceID == work.id)
    }
    ...
    @Test func reopeningAnEmptyClosedWorkspaceKeepsTheSelectionAndTarget() {
        let (store, recentClosed, _) = makeStoreWithRecentClosed()
        let work = store.addWorkspace(name: "work")
        let session = store.addSession(toWorkspace: work.id, cwd: "/a")!
        let second = store.addSession(toWorkspace: work.id, cwd: "/b")!
        ...
        store.finalizePendingClose(store.pendingCloseSummary!.id)
    ...
    @Test func selectingAWorkspaceDropsTheFreshWorkspaceAsCurrent() {
        let store = makeStore()
        let work = store.addWorkspace(name: "work")
        let session = store.addSession(toWorkspace: work.id, cwd: "/a")!
        store.selectSession(session.id)
    ...
    @Test func selectingAnEmptyWorkspaceStillMakesItCurrent() {
        let store = makeStore()
        let work = store.addWorkspace(name: "work")
        let session = store.addSession(toWorkspace: work.id, cwd: "/a")!
        store.selectSession(session.id)
    ...
    @Test func selectingAFilteredOutEmptyWorkspaceRevealsIt() {
        let store = makeStore()
        let work = store.addWorkspace(name: "work")
        let session = store.addSession(toWorkspace: work.id, cwd: "/a")!
        store.selectSession(session.id)
    ...
    @Test func filteringOutTheFreshWorkspaceDropsItForGood() {
        let store = makeStore()
        let work = store.addWorkspace(name: "work")
        let session = store.addSession(toWorkspace: work.id, cwd: "/a")!
        store.selectSession(session.id)
