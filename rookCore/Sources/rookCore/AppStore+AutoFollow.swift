...
extension Notification.Name {
    ...
    /// blocked session, carrying the target session id under `AppStore.autoFollowSessionIDKey`. rookCore
    ...
    public static let rookAutoFollowed = Notification.Name("rook.autoFollowed")
    ...
    public static let rookSidebarVisibilityChanged = Notification.Name("rook.sidebarVisibilityChanged")
}
...
extension AppStore {
    /// The `userInfo` key carrying the auto-followed session's `UUID` in an `.rookAutoFollowed`
    ...
    /// can drive it directly. Posts `.rookAutoFollowed` after the select so the app target can move first
