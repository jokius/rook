...
struct SessionTests {
    ...
    @Test func effectiveCwdStaysPrimaryWhileSplitFocused() {
        // effectiveCwd (new-pane seeding + ROOK_SESSION_PWD) is NOT focus-aware.
        let session = Session(initialCwd: "/repo")
        session.currentCwd = "/repo/primary"
        session.isSplit = true
        session.splitFocused = true
    ...
    @Test func searchFieldsAreNotPersistedAcrossSnapshot() {
        ...
        let store = AppStore(persistence: PersistenceStore(
            directory: FileManager.default.temporaryDirectory.appendingPathComponent("rook-tests-\(UUID().uuidString)")))
    ...
    @Test func paneRoleFollowsAPromotedSurvivorAndReSplit() {
        // a split survivor (baked `right`, token `agent-tok`) is promoted into the MAIN slot, then a fresh
        // helper (also baked `right`, token `helper-tok`) opens as the new split. Both shells were baked
        // with the SAME stale `right` role, so only the stable token disambiguates them: the survivor now
        // resolves to the main slot (.left), the helper to the split slot (.right).
        let session = Session(initialCwd: "/repo")
