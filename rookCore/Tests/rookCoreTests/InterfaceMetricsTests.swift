...
/// One window/sidebar/size combination the panel fitting must survive.
...
struct InterfaceMetricsTests {
    @Test func defaultSizeReproducesTheHardcodedChromeFonts() {
        ...
        #expect(metrics.scaled(520) == 520) // the palette's panel width
        #expect(metrics.scaled(320) == 320) // its results height, and the popovers' width
    }
    ...
    /// The regression the fitting exists for: the scaled width and the centering offset each grow
    /// unbounded and COMPOUND, so the panel's right edge ran past the window. Every case here is one the
    /// shipped defaults or the settings ranges allow (sidebar 160...560 + 1pt divider, window minimum
    /// 640x400, default 900x600).
    ...
    func fittedPanelStaysInsideTheWindow(_ testCase: PanelCase) {
        for ideal in [520.0, 460.0] { // the palette and the switcher
    ...
    @Test func panelOffsetDegradesToWholeWindowCenteringRatherThanClipping() {
        ...
        // no sidebar on screen: no shift at all, whatever the window
    ...
    /// Pins the decision behind the switcher panel's height. The RENDERED result has no harness on either
    /// side — hosted tests do not render SwiftUI and XCUITest cannot hold Ctrl for the switcher — so this
    /// covers the choice, and the trap it exists for is named on `measuredPanelHeight`.
