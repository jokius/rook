...
private let logger = Logger(subsystem: "com.rook.app", category: "NotificationManager")
...
final class NotificationManager: NSObject, @preconcurrency UNUserNotificationCenterDelegate {
    ...
    /// `rookApp`; weak since `AppActions` outlives the manager only by app lifetime.
    ...
    /// `.criticalRequest` macOS auto-cancels when rook becomes active — both no-op while rook is the
    ...
    /// suppressed for rook without it. `.sound` is what lets the configured notification sound
    ...
    /// session + pane, suppresses when that pane is focused and rook is active, otherwise posts a
    ...
    /// defaults, the global `~/.config/ghostty/config`, the rook-scoped `ghostty.conf`, and the UI
    ...
    /// `.informationalRequest`; `.untilFocused` a `.criticalRequest` that bounces until rook becomes active
    /// (macOS auto-cancels it then, so there is no cancel bookkeeping). Both are automatically a no-op while
    /// rook is the active app, so a notification for a session you're already looking at never bounces.
    ...
    /// Present banners — with their attached sound — even while rook is the active app (the
    ...
    /// looking at would only ding while rook is backgrounded.
    func userNotificationCenter(_: UNUserNotificationCenter, willPresent _: UNNotification,
                                withCompletionHandler completionHandler: @escaping (UNNotificationPresentationOptions) -> Void) {
        completionHandler([.banner, .list, .sound])
    }

    /// A banner was clicked: bring rook forward and navigate to the firing session/pane (decoded from
