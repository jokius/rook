...
/// menus, move, close) the rookCore unit tests cannot reach.
///
...
final class SidebarUITests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        markerDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("rook-sidebar-\(UUID().uuidString)", isDirectory: true)
    ...
    /// With `workspaceRowClickExpands` off, only the disclosure triangle stays a toggle. Covers the
    /// LAUNCH-time mirror leg (a seeded `settings.json` read at `SettingsModel.init`).
    ...
    /// Opens Settings (⌘,) if needed, switches to `tab`, and returns the control with `control` id once it
    /// is hittable — retrying the tab click each tick. Mirrors the `SettingsUITests` helper: a reopened,
    /// non-key Settings window silently swallows the first tab click.
    @discardableResult
