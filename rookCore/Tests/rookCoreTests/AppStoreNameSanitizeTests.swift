...
/// Control-character sanitizing on every store arm whose value reaches an `{AGT_*}` custom-command token —
/// those expand unquoted into `/bin/sh -c`, so an interior newline is a statement separator.
...
struct AppStoreNameSanitizeTests {
    @Test func addSessionStripsInteriorControlCharactersFromNameAndCwd() {
        ...
        let session = store.addSession(toWorkspace: ws.id,
                                       cwd: "/work\ntouch /tmp/pwned",
                                       name: "demo\ntouch /tmp/pwned")!
    ...
    @Test func ensureWorkspaceWithControlCharacterNameStaysIdempotent() {
        ...
        let store = makeStore()
        let first = store.ensureWorkspace(named: "prod\ntouch /tmp/pwned")!
        let second = store.ensureWorkspace(named: "prod\ntouch /tmp/pwned")!
