...
/// Content view whose hit resolution the test supplies directly, standing in for the real window hierarchy:
/// `ownsPointer` only asks WHO owns a point, so the geometry that would produce that answer is beside the
/// point here.
...
/// Pins the chrome-versus-surface split that keeps the resize cursor over the dividers. Returning true for
/// chrome puts the cursor flicker back; returning false for a sibling pane silences the visible terminal's
/// own cursor instead. Neither shows up in any other test.
@MainActor
final class GhosttySurfaceViewTrackingTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        await MainActor.run {
            ...
            // every surface stays at its zero init frame, so `viewDidMoveToWindow` parks in
    ...
    /// A split's other pane is on screen too, so owning the point keeps its own cursor writer alive — and the
    /// eager deck stacks every session at one frame, so a hit can resolve to a hidden session's surface just
    /// as easily. That is `deckVisible`'s question, not this one.
    ...
    func testTeardownRemovesTheHudBodyFile() throws {
        let body = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("rook-hud-teardown-\(UUID().uuidString).txt")
