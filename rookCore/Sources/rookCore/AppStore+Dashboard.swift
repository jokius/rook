...
extension AppStore {
    ...
    /// `.primary` cell plus a `.split` cell when it `hasSplit` (both shells alive, shown OR hidden), so a bare
    /// id makes a split session two cells; an explicit pane yields that cell alone. Ids that don't resolve to
    /// a session are skipped, and repeated cells collapse, so a bare id alongside a pane ref for the same
    /// session cannot double-host one surface. UNCAPPED: the caller applies the cell cap and reports the
    /// dropped-pane count, so the expansion has exactly ONE implementation shared by
    /// `ControlServer.setDashboard` and `AppActions.toggleDashboard`.
    ///
    ...
    /// target may name ONE pane of a split session.
