...
public enum SidebarDrop {
    ...
    /// the current workspace. With nothing marked, the filter off, or two or more members there is no such
    ...
    /// The inverse of `workspaceInsertIndex`: maps a `workspaces`-index insert point back to the RENDERED
    /// slot, i.e. how many visible workspace rows sit above it. `NSOutlineView.setDropItem` counts rendered
    /// children, so the drop HIGHLIGHT needs this even though the move itself is computed in store space —
    /// without it a non-contiguous marked set draws the insertion line a row off from where the drop lands.
    /// The identity while every workspace is visible, which is why the single-mark filter never exposed it.
    public static func workspaceHighlightSlot(visibleIndices: [Int], insertIndex: Int) -> Int {
        visibleIndices.reduce(0) { $0 + ($1 < insertIndex ? 1 : 0) }
    }
