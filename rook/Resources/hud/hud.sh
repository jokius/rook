#!/bin/sh
# hud.sh — the panel painter rook runs in a session's overlay slot for `session.hud.*`.
#
# One HUD-SPECIFIC environment variable, ROOK_HUD_FILE, points at the rendered body the app writes (the
# surface also inherits the session environment and the overlay wrapper's own ROOK_OVL_* pair):
#
