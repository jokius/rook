...
extension WindowContentView {
    ...
    /// The dashboard caption pill's FILL — rook's brand selection pill (`rookGreen`), the same color the
    /// selected sidebar row draws, so the caption chip reads as "selected" exactly like a sidebar row.
    /// (Upstream resolves the theme's selection background here; rook's sidebar pill is brand-colored, not
    /// theme-derived, so the dashboard follows the sidebar.)
    private var dashboardPillColor: Color { Color(nsColor: .rookGreen) }

    /// The dashboard caption pill's TEXT — the sidebar pill's foreground (`rookGraphite`), readable over
    /// `dashboardPillColor`.
    private var dashboardPillTextColor: Color { Color(nsColor: .rookGraphite) }
