...
/// `path_helper`. Without widening, a bare `rookctl`, `jq` or `lazygit` in a keymap line exits 127 with
/// the shell's own diagnostic discarded.
public enum CommandPath {
    ...
    /// `ROOK_STATE_DIR`, then Application Support resolve that identically for either binary. The standard
