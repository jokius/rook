...
/// app bundle into `~/.config/rook/agent-status/`, bakes the bundled `rookctl`'s absolute path
...
/// is configured. Claude/Codex configs write a `.bak` first; the Codex step is gated on `~/.codex`
...
/// transforms and Pi ownership policy live in `rookCore.AgentHooksInstall`; this type owns the AppKit
/// filesystem glue.
/// Idempotent and re-runnable: re-running refreshes the baked `rookctl` path (healing a
/// moved/reinstalled bundle) and the managed Pi extension, and is a clean no-op for already-present
/// rc/settings/config entries.
@MainActor
enum AgentHooksInstaller {
    ...
    /// The bundled `rookctl` at `Contents/MacOS/rookctl`, or nil when this build skipped bundling.
    private static var bundledTool: URL? { Bundle.main.url(forAuxiliaryExecutable: CLIInstall.toolName) }

    /// The install destination, `~/.config/rook/agent-status/`.
    private static var destinationFolder: URL {
        FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".config/rook/agent-status")
    }
    ...
    private enum CodexResult {
        ...
        // a warning-level outcome: rook could not auto-merge and the user must act (add the block by
    ...
    // informational; the warning cases mean rook could not safely write and left the destination as-is.
    private enum PiResult {
        case installed, alreadyConfigured, userOwned, unreadable, writeFailed, noPi

        // a warning-level outcome: rook could not update the extension and the user must act (move the
    ...
    // copy the bundled agent-status folder into ~/.config/rook/agent-status, overwriting any prior
    ...
    private static let rookctlMarker = "# >>> rook rookctl path (installer-baked) >>>"

    // bake the bundled rookctl's absolute path into the installed wrappers so the hooks fire even when
    // the CLI was never symlinked into PATH. `[ -n "${ROOKCTL:-}" ] || ROOKCTL='<path>'` sets it only
    // when ROOKCTL is unset, so an explicit env override still wins (resolution order 1 > 2 > PATH); the
    ...
    private static func bakeRookctlPath() throws {
        ...
        for name in [AgentHooksInstall.wrapperName, AgentHooksInstall.codexWrapperName,
            ...
            let block = rookctlMarker + "\n[ -n \"${ROOKCTL:-}\" ] || ROOKCTL=\(AgentHooksInstall.shellQuote(tool.path))\n"
            let baked = insertAfterShebang(stripped, block: block)
            try writePreservingSymlink(baked, to: wrapper)
        }
    }

    // remove a previously baked ROOKCTL block (the marker line plus the assignment line under it).
    private static func stripBakedBlock(from text: String) -> String {
        ...
        for line in lines {
            ...
            if line == rookctlMarker { skip = 1; continue } // drop the marker and the assignment below it
            result.append(line)
        }
    ...
    // same-named, unmarked extension is user-owned and left untouched; a rook-managed one refreshes from
    // the newly copied package. Pi has no config merge, and its extension carries no user state, so unlike
    // Claude/Codex config we do not create a backup for a managed refresh.
    ...
    private static func piText(_ pi: PiResult) -> String {
        switch pi {
        case .installed:
            return "Pi lifecycle extension installed to ~/.pi/agent/extensions/rook-status.ts. Restart Pi or run /reload."
        case .alreadyConfigured:
            return "Pi lifecycle extension is already current at ~/.pi/agent/extensions/rook-status.ts."
        case .userOwned:
            return "~/.pi/agent/extensions/rook-status.ts is user-owned, so rook left it untouched."
        case .unreadable:
            return "~/.pi/agent/extensions/rook-status.ts exists but could not be read, so rook left it untouched."
        case .writeFailed:
    ...
    private static func codexText(_ codex: CodexResult) -> String {
        ...
        case .hooksExist:
            return "Your ~/.codex/config.toml already defines its own hooks, so rook left it untouched. "
                + "Add these lifecycle hooks yourself, then run /hooks in Codex:\n\n" + codexBlock
        case .unparseable:
            return "Your ~/.codex/config.toml isn't valid TOML, so rook left it untouched. "
                + "Fix it, or add these lifecycle hooks yourself, then run /hooks in Codex:\n\n" + codexBlock
        case .unreadable:
            return "Your ~/.codex/config.toml exists but couldn't be read, so rook left it untouched."
        case .noCodex:
