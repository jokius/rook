...
struct AgentHooksInstallTests {
    private let scriptDir = "/Users/me/.config/rook/agent-status"
    ...
    @Test func mergePreservesUnrelatedHooksAndKeys() throws {
        ...
        // UserPromptSubmit keeps the other hook AND gains the rook one
    ...
    @Test func codexHooksBlockMapsActionsAndBakesWrapperPath() {
        let block = AgentHooksInstall.codexHooksBlock(scriptDir: scriptDir)
        let hook = scriptDir + "/rook-codex-status.sh"
        // Codex-specific behavior stays in the installed Codex hook. rook only generates the six
    ...
    @Test func codexHooksBlockShellQuotesPathWithSpace() {
        let dir = "/Users/my name/.config/rook/agent-status"
        ...
        #expect(block.contains("command = \"'\(dir)/rook-codex-status.sh' session-start\""))
    }
    ...
    @Test func codexHooksBlockEscapesApostropheInPath() {
        ...
        #expect(block.contains("'/Users/O'\\\\''Brien/agent-status/rook-codex-status.sh' session-start"))
    }
    ...
    @Test func mergeCodexConfigStripsLegacyNotifyLine() {
        let existing = "notify = [\"/Users/me/.config/rook/agent-status/codex-notify.sh\"]\n"
        let contents = mergedContents(AgentHooksInstall.mergeCodexConfig(existing: existing, scriptDir: scriptDir))
    ...
    @Test func piExtensionPathsUsePiGlobalExtensionsDirectory() {
        ...
        #expect(AgentHooksInstall.piExtensionPath(home: "/Users/me") == "/Users/me/.pi/agent/extensions/rook-status.ts")
    }
    ...
    @Test func piExtensionOwnershipProtectsUserExtension() {
        ...
        #expect(AgentHooksInstall.mayOverwritePiExtension(
            fileExists: true,
            existingContents: "// rook-pi-status-extension\nexport default () => {}\n"
        ))
    ...
    private func makeTempDir() throws -> URL {
        let dir = FileManager.default.temporaryDirectory.appendingPathComponent("rook-hooks-\(UUID().uuidString)")
