...
/// rookCore when that preserves the old response order; target-dependent parsing stays here with
...
extension ControlServer: ControlActions {
    ...
    /// then `.rookApplySplitRatio` pokes the session's `SplitProbeView` to move the live divider (a no-op
    ...
    func resizeSplit(_ target: String?, window: String?, resize: ControlSplitResize) -> ControlResponse {
        return resolver.resolveSession(target, window: window) { store, id in
            ...
            NotificationCenter.default.post(name: .rookApplySplitRatio, object: session)
    ...
    /// across ALL config sources (bundled defaults, the global `~/.config/ghostty/config`, the rook-scoped
    ...
    /// ("default ghostty"), NOT the seeded `rook` app default; any other name must be a bundled theme, else
    ...
    /// is a `#rrggbb` glyph-tint override and `update.shape` the matching silhouette override (both validated in the dispatcher);
    /// each rides the ephemeral indicator, so it lasts only until the next `session.status` without it. `update.pane`
