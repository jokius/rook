...
final class SplitUITests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        markerDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("rook-split-\(UUID().uuidString)", isDirectory: true)
    ...
    // promote → re-split → exit-main: driven through the REAL pane-exit routing (typing `exit`, not calling
    // closePrimaryPane directly like the host-free test). After the primary exits, the right pane promotes
    // into the sole/main slot; a fresh split then opens a new right pane; exiting the promoted MAIN pane
    // must route through closePrimaryPane (dispatched on the surface's LIVE role, now primary) and collapse
    // onto the FRESH right pane — not through the survivor's stale split onExit, which would tear the fresh
    // right down and strand the dead main. The tell: the final command reaches the fresh right shell. This
    // is the only test that guards the role-aware dispatch; reverting it strands the session.
    func testPromoteThenResplitThenExitMainCollapsesToFreshSplit() throws {
        ...
        // NOT closeSplitPane, which would tear the fresh right down and strand the dead main.
        app.typeKey(.leftArrow, modifierFlags: [.command, .option])
        usleep(500_000)
        app.typeText("exit")
        app.typeKey(.return, modifierFlags: [])
