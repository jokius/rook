...
/// The first-launch alert pointing at the Help menu extras, with checkboxes for the two a Homebrew install
/// does not provide. Host-free copy and the due-decision are `rookCore.FirstRunWelcome`.
@MainActor
enum WelcomeAlert {
    ...
    /// failed install cannot bring it back on the next launch. Suppressed under XCUITest, since every test
    /// launches on a fresh isolated state directory and would otherwise open a modal before its first
    /// assertion.
    static func presentOnce(stateDirectory: URL) {
        guard !presented, !ContentView.isUITestLaunch else { return }
        presented = true
        FirstRunWelcome.markShown(in: stateDirectory)
    ...
    /// The alert and its two option checkboxes, laid out and aligned.
