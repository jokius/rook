...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    /// a folder path passed by `open -a rook /path`. Nil before the scene `.task` runs.
    var actions: AppActions?

    /// Directories requested via `open -a rook /path` (the OS "open terminal here" integration) that
    ...
    func applicationWillFinishLaunching(_: Notification) {
        // rook has its own multi-window model and does NOT support native window tabs; disabling
    ...
    /// rook's own item uses a SwiftUI closure action (a different selector), so only the native item
    ...
    /// `open -a rook /path` (the OS "open terminal here" integration): macOS delivers the folder/file
    ...
    /// This serves the WARM case — rook already running (its daily-driver norm) — where the running
