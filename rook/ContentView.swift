...
struct ContentView: View {
    ...
    /// True when running under an isolated XCUITest (`ROOK_STATE_DIR` set AND the
    /// `ROOK_UITEST_FORCE_SIDEBAR_VISIBLE` env sentinel present). Gates the FB11763863 window-present
    ...
    static var isUITestLaunch: Bool {
        ...
        return process.environment["ROOK_STATE_DIR"] != nil
            && process.environment["ROOK_UITEST_FORCE_SIDEBAR_VISIBLE"] != nil
    }
    ...
    static var shouldBypassCloseConfirmation: Bool {
        isUITestLaunch && ProcessInfo.processInfo.environment["ROOK_UITEST_ALLOW_CLOSE_CONFIRMATION"] == nil
    }
