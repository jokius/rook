...
struct ConfigPathsTests {
    ...
    @Test func defaultWhenNeitherSettingNorStateDir() {
        ...
        #expect(dir.path == "/Users/test/.config/rook")
    }
    ...
    @Test func emptyStringsFallThrough() {
        ...
        #expect(ConfigPaths.configDirectory(setting: "", stateDir: "", home: home).path == "/Users/test/.config/rook")
    }

    @Test func keymapPathIsKeymapConfInDir() {
        let dir = URL(fileURLWithPath: "/Users/test/.config/rook")
        #expect(ConfigPaths.keymapPath(configDirectory: dir).path == "/Users/test/.config/rook/keymap.conf")
    }

    @Test func starterKeymapConfIsCommentedAndListsActions() {
        let starter = ConfigPaths.starterKeymapConf()
        #expect(starter.contains("rook keymap — a kitty-flavored config"))
        ...
        #expect(starter.contains("command \"Lazygit\"      ctrl+a>g     rookctl session overlay open 'zsh -lc lazygit' --socket \"$AGT_SOCKET\""))
    ...
    @Test func ghosttyConfigPathIsGhosttyConfInDir() {
        let dir = URL(fileURLWithPath: "/Users/test/.config/rook")
        #expect(ConfigPaths.ghosttyConfigPath(configDirectory: dir).path == "/Users/test/.config/rook/ghostty.conf")
    }

    @Test func restoreDenylistPathIsRestoreDenylistConfInDir() {
        let dir = URL(fileURLWithPath: "/Users/test/.config/rook")
        #expect(ConfigPaths.restoreDenylistPath(configDirectory: dir).path == "/Users/test/.config/rook/restore-denylist.conf")
    }
    ...
    @Test func editorCommandRunsThroughInteractiveLoginShellThenPosixSh() {
        ...
        #expect(ConfigPaths.editorCommand(forPath: "/Users/test/.config/rook/keymap.conf")
                == "${SHELL:-/bin/zsh} -ilc 'exec /bin/sh -c '\\''${VISUAL:-${EDITOR:-vi}} \"$1\"'\\'' rook-config-edit '\\''/Users/test/.config/rook/keymap.conf'\\'''")
    }
    ...
    @Test func editorCommandEmbedsAnyPathForBothEditorOverlays() {
        ...
        #expect(cmd.contains("rook-config-edit '\\''/Users/test/.config/rook/ghostty.conf'\\'''"))
    }
    ...
    private func makeTmp() throws -> URL {
        let tmp = FileManager.default.temporaryDirectory.appendingPathComponent("rook-editorcmd-\(UUID().uuidString)")
