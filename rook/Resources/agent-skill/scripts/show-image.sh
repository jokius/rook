#!/bin/bash
# show-image.sh — display an image inside the current rook session.
#
...
#
# requires running inside rook (ROOK_ENABLED=1) with rookctl on PATH.
...
command -v rookctl >/dev/null 2>&1 || { echo "show-image.sh: rookctl not on PATH" >&2; exit 1; }
[ "${ROOK_ENABLED:-}" = "1" ] || { echo "show-image.sh: not inside rook; try: open \"$img\"" >&2; exit 1; }
...
rookctl session overlay open "/bin/bash $(sq "$self") --emit $(sq "$abs")" --size-percent "$size"
