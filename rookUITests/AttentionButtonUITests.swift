...
final class AttentionButtonUITests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        stateDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("rook-attnuitest-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: stateDir, withIntermediateDirectories: true)
        // the runner's own temp dir keeps the socket path under the sun_path ~104-byte limit and inside
        // the sandbox grant (the per-test ROOK_STATE_DIR subdir is too long; /tmp is outside the grant).
    ...
    // flipping the Settings toggle at runtime shows/hides the bell live (the .rookAppearanceChanged
