...
struct SessionSwitcherOverlay: View {
    ...
    /// the whole window. This scrim is deliberately click-through — the switch ends on Ctrl release.
    ...
    /// Ctrl-Tab, so it can never render a stale size.
    ...
    /// The panel width at the 13pt default; `metrics` scales it, then the window fits it.
    ...
    /// The rows SCROLL rather than overflow: ten candidates at a large interface size want more height
    /// than a short window has, and a plain stack has no other way to give. `scrollTo` keeps the cycling
    /// selection visible, which is the whole point of the overlay.
    ///
    /// The height is MEASURED and then capped, never proposed: a `ScrollView` takes whatever height it is
    /// offered instead of sizing to its content, so handing `maxRowsHeight` straight to
    /// `.frame(maxHeight:)` makes every panel that tall — three rows in a tall window would draw a panel
    /// of mostly empty material. `rowsHeight` starts at 0, where the frame stays unconstrained for the one
    /// layout pass before the preference lands.
    private func panel(maxRowsHeight: Double) -> some View {
        ScrollViewReader { proxy in
            ...
            .onPreferenceChange(RowsHeightKey.self) { height in rowsHeight = height }
    ...
    /// Same rule as the command palette's panel: native material, or a solid window color under Reduce
    /// Transparency. Effective-only — no settings are written.
...
struct SessionSwitcherRow: View {
    ...
    /// Read from the non-observable `GhosttyApp` — both the Ctrl-Tab overlay and the title-bar popovers
    /// mount fresh on every open, so neither renders a stale size.
    private let metrics = InterfaceMetrics.current
