...
struct AppStoreOrganizationTests {
    ...
    @Test func setSameValuesAreNoOpWritesAndStable() {
        ...
        #expect(a.flagged)
        #expect(store.sidebarMode == .flagged) // state stable across the no-op setters
        // the focus mutators' own no-op-never-writes pass is in AppStoreFocusTests, over every mode.
    }
