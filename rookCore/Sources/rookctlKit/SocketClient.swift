...
/// A blocking, one-request-per-connection client for the rook control socket: connect to a unix domain
...
struct SocketClient {
    ...
    private func connect() throws -> Int32 {
        ...
        guard result == 0 else {
            ...
            close(fd)
            throw SocketClientError("connect(\(path)) failed: \(message) — is rook running?")
        }
        return fd
    }
