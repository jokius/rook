#!/usr/bin/env bash
# Codex lifecycle hook installed by rook's Help ▸ Install Agent Status Hooks command.
#
...
# agent-specific workaround inside the installed hook package: one watcher per rook pane reads the
# live visible footer and reports blocked only after Codex displays an approval or question dialog.
set -u

[ -n "${ROOK_SESSION_ID:-}" ] || exit 0
...
status_wrapper=${ROOK_STATUS_WRAPPER:-"$script_dir/rook-agent-status.sh"}
...
read_visible_screen() {
  "${ROOKCTL:-rookctl}" session text \
    --target "$ROOK_SESSION_ID" \
    "${socket_args[@]+"${socket_args[@]}"}" \
    "${pane_args[@]+"${pane_args[@]}"}" 2>/dev/null
}

watch_file_path() {
  if [ -n "${ROOK_CODEX_WATCH_FILE:-}" ]; then
    printf '%s' "$ROOK_CODEX_WATCH_FILE"
    return
  fi
  local key
  key=$(printf '%s-%s' "$ROOK_SESSION_ID" "${ROOK_PANE:-left}" | /usr/bin/tr -c 'A-Za-z0-9._-' '_')
  printf '%s/rook-codex-watch-%s-%s' "${TMPDIR:-/tmp}" "${UID:-0}" "$key"
}
...
watch_for_blocker() {
  ...
  local max_checks=${ROOK_CODEX_WATCH_MAX_CHECKS:-21600}
  local interval=${ROOK_CODEX_WATCH_INTERVAL:-0.5}
