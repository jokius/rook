# rook-agent-status — shell integration for bash and zsh.
...
#   export ROOK_AGENT_RE='^(gemini|cursor-agent|my-agent)([[:space:]]|$)'
#
# Claude Code, Codex, and Pi are intentionally NOT in the default list — their own
# hooks/extensions drive finer per-turn state, which the coarse process-level active/idle
# here would only fight. Add any of them here if you rely on the shell integration
# alone for it.
#
...
[ -n "${ROOK_SESSION_ID:-}" ] || return 0 2>/dev/null

# Locate rook-agent-status.sh relative to this file (layout is preserved on
# install: shell/integration.sh sits next to rook-agent-status.sh's dir).
...
: "${ROOK_AGENT_BIN:=$_ags_dir/rook-agent-status.sh}"
: "${ROOK_AGENT_RE:=^(gemini|cursor-agent|aider|opencode|crush|goose)([[:space:]]|$)}"

if [ -n "${ZSH_VERSION:-}" ]; then
  autoload -Uz add-zsh-hook
  _ags_preexec() { [[ "$1" =~ $ROOK_AGENT_RE ]] && { "$ROOK_AGENT_BIN" active --blink; _ags_active=1; }; }
  _ags_precmd()  { [[ -n "${_ags_active:-}" ]] && { "$ROOK_AGENT_BIN" idle; unset _ags_active; }; }
  ...
  if [ -n "${bash_preexec_imported:-}${__bp_imported:-}" ]; then
    ...
    case "$(trap -p DEBUG 2>/dev/null)" in
      ...
      *) printf '%s\n' "rook agent-status: an existing bash DEBUG trap is set; skipping the active-state hook (use bash-preexec, or wire a preexec yourself)." >&2 ;;
    esac
