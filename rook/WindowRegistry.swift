...
final class WindowRegistry {
    ...
    func register(_ id: WindowInfo.ID, window: NSWindow) {
        windows[id] = window
        NotificationCenter.default.post(name: .rookWindowAttachmentChanged, object: nil)
    }
...
extension Notification.Name {
    ...
    /// the first `didBecomeKey` is a no-change and skips `.rookWindowFrontmostChanged`, and a brand-new
    /// window has no saved frame to restore, so no `didMove`/`didResize` fires either.
    static let rookWindowAttachmentChanged = Notification.Name("rook.windowAttachmentChanged")
}

// CoreGraphics <-> host-free WindowGeometry conversions, kept app-side: rookCore stays Foundation-only
