...
private let logger = Logger(subsystem: "com.rook.app", category: "SettingsModel")
...
final class SettingsModel {
    ...
    /// Coalesces `.rookSystemAppearanceChanged` into a single re-apply when the macOS light/dark
    ...
    init(library: WindowLibrary, settingsStore: SettingsStore) {
        ...
        // theme (rook) lives only in memory (load() seeds it; it isn't in settings.json), so without
        ...
        NotificationCenter.default.addObserver(forName: .rookSystemAppearanceChanged, object: nil,
    ...
    /// `.rookSystemAppearanceChanged` on the same side (KVO `[.initial]` seeds one at launch, and the
    ...
    /// default): rook stays self-contained; the scoped `ghostty.conf` is the place for customizations.
    ...
    /// Re-read and re-parse `keymap.conf`, then post `.rookKeymapChanged` so the custom-command
    ...
    func reloadKeymap() {
        loadKeymap()
        NotificationCenter.default.post(name: .rookKeymapChanged, object: nil)
        if !keymapDiagnostics.isEmpty {
            NotificationManager.shared.notifyKeymapDiagnostics(count: keymapDiagnostics.count)
        }
    }

    /// The resolved config directory: the explicit setting, else `ROOK_STATE_DIR/config` (test
    /// isolation), else `~/.config/rook`. Both `keymap.conf` and `ghostty.conf` live here.
    ...
    /// The resolved rook-scoped `ghostty.conf` path: `<config dir>/ghostty.conf`, co-located with
    /// `keymap.conf`.
    ...
    /// Rebuild the ghostty config from all sources (re-reading the rook-scoped `ghostty.conf` the user
    ...
    /// re-read. Posts `.rookAppearanceChanged` so the chrome picks up any color change, and a warning
    ...
    /// example override, and a note that rook's UI-managed keys win over this file. Every line is a
    /// comment so a fresh file changes nothing.
    private func starterGhosttyConfigText() -> String {
        """
        # rook-scoped ghostty config — applies ONLY to rook, not the standalone Ghostty.app.
        # Put any ghostty config key here to override rook's bundled defaults and your global
        ...
        #
        # NOTE: rook's UI-managed keys (font, theme, background opacity/blur, scroll speed) are set
