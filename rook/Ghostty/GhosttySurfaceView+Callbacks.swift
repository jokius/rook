...
extension GhosttySurfaceView {
    ...
    /// surface, retaining the config for teardown. The scratch surface inherits its owner's visual config
    /// through `watermarkSession` while staying operationally sessionless; the overlay/quick-terminal
    /// surfaces carry neither link and are a no-op. A nil watermark with no font override
    ...
    func applyWatermarkFromSession() {
        ...
        // opacity / dashboard re-assert paths guard on the latch BEFORE calling this, so they never reach
        // here with a live OSC to drop.
    ...
    /// sessionless scratch/overlay whose live cmd-+/- zoom would otherwise reset. A malformed hex is
    /// rejected. Retains the per-surface config in `ownedConfigs`, freed on teardown.
    func applyOSCBackground(_ hex: String) {
        ...
        let overlay = WatermarkConfig.overlayText(watermark: BackgroundWatermark(kind: .color, colorHex: hex),
                                                  ...
                                                  fontSize: dashboardFontOverride ?? session?.fontSize
                                                      ?? currentFontSize() ?? initialFontSize.map(Double.init),
                                                  windowOpacity: GhosttyApp.shared.windowOpacity)
        guard let config = GhosttyApp.shared.configWithOverlay(overlay) else {
