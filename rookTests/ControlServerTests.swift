...
/// Hosted coverage for `ControlServer`'s bind lifecycle: who owns the socket path, and what a refused
/// instance advertises. The server is `@MainActor` and needs the real window library, so this cannot move
/// to `rookCore`.
///
/// The fixture follows the three constraints the control-api rule records (see
/// `ControlServerHudResizeTests`): the `WindowLibrary` is rooted in a throwaway directory, `SettingsModel`
/// takes the DEFAULT `SettingsStore`, and `start()` is called ONLY on an explicit short `/tmp` path — never
/// the default one, which would displace the deployed app's socket.
@MainActor
final class ControlServerTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        try await MainActor.run {
            stateDir = FileManager.default.temporaryDirectory
                .appendingPathComponent("rook-control-server-tests-\(UUID().uuidString)", isDirectory: true)
            try FileManager.default.createDirectory(at: stateDir, withIntermediateDirectories: true)
            // NOT under stateDir: the hosted app's temporary-directory path alone can exceed sun_path's
            // ~104 bytes, which start() refuses before it reaches anything this file tests.
            socketPath = "/tmp/rook-test-\(UUID().uuidString.prefix(8)).sock"
        }
    }
    ...
    /// The refusal is only half the guard: `stop()` returning early on the refused instance is what stops it
    /// unlinking the owner's socket on quit, which is the second half of the original orphaning.
    ...
    /// A refused instance must not advertise a path it does not serve: every shell it spawns would carry the
    /// OTHER instance's socket in `ROOK_SOCKET` and drive that app instead. It advertises an unbindable path
    /// rather than nothing, because the status hooks read an ABSENT variable as "resolve the default", which
    /// is that same other instance.
    ///
    /// Refusal must be known WITHOUT `start()`: the launch window's surfaces snapshot the environment during
    /// the initial render pass, and `start()` only runs from the scene's `.task` afterwards, so a refusal
    /// decided there would reach that first shell too late.
    ...
    func testStartRefusesAnOwnerWhoseBacklogIsSaturated() {
        ...
        // ControlServer's read timeout, so everything after it queues until the backlog is full.
        var clients: [Int32] = []
