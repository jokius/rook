...
/// directory (honoring `ROOK_STATE_DIR` for test isolation, like the snapshot and the watermark PNGs).
...
///
/// Each function takes an optional `stateDir` override (default nil = the `ROOK_STATE_DIR`/app-support
...
public enum WorkspaceIconStorage {
    ...
    public static func directoryURL(stateDir: URL? = nil) -> URL {
        let base = stateDir
            ?? ProcessInfo.processInfo.environment["ROOK_STATE_DIR"].map { URL(fileURLWithPath: $0, isDirectory: true) }
