...
public enum WatermarkConfig {
    ...
    /// Valid `#rrggbb` hex color — exactly what `NSColor(rookHex:)` parses (an optional leading `#` then
    ...
    /// that `NSColor(rookHex:)`'s `UInt32(radix:)` cannot parse, which would pass validation and then
