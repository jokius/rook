...
final class ControlHudUITests: ControlAPITestCase {
    ...
    func testHudOpenReportsItsMessageAndNeverAProgramOverlay() throws {
        ...
        let width = try XCTUnwrap(hud["sizePercent"] as? Int,
                                  "the panel's measured share of the pane should be reported")
        ...
        let node = try XCTUnwrap(sessionNode(session))
    ...
    func testHudUpdateRewritesTheReadBackInPlace() throws {
        ...
        XCTAssertEqual(try XCTUnwrap(sessionNode(session))["overlay"] as? Bool, false,
    ...
    func testHudCloseClearsBothTheHudAndTheOverlaySlot() throws {
        ...
        XCTAssertTrue(pollHudGone(session), "tree should omit hud once it is closed")
        let node = try XCTUnwrap(sessionNode(session))
    ...
    func testTheSessionKeepsKeyboardFocusWhileAHudIsUpAndWhenTheHudIsClicked() throws {
        ...
        // the click below aims at the detail area's centre, so the panel has to be tall enough to cover it:
        // the width takes the maximum and the HEIGHT only follows the message, so the message supplies rows.
    ...
    /// Types `command` + Return via the real keyboard, retrying until `file` reports a non-empty marker. The
    /// keyboard routes to whatever holds first responder, which is the whole point of the assertion: a
    /// stolen focus writes nothing, and a re-typed `cmd > file` is idempotent.
    ...
    /// The session's `tree` node, matched case-insensitively: the socket echoes ids in whichever case the
    /// caller used, and `activeSessionID` reads one from the same response.
    ...
    private func hudNode(_ id: String) -> [String: Any]? { sessionNode(id)?["hud"] as? [String: Any] }
    ...
    private func pollHudGone(_ id: String, timeout: TimeInterval = 10) -> Bool {
        let deadline = Date().addingTimeInterval(timeout)
        repeat {
            if hudNode(id) == nil { return true }
            usleep(150_000)
        } while Date() < deadline
        return false
    }
