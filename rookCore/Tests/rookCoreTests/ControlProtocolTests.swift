...
struct ControlProtocolTests {
    ...
    @Test func workspaceCollapseExpandRoundTrip() throws {
        ...
        #expect(try roundTrip(collapse) == collapse)
        let expand = ControlRequest(cmd: .workspaceExpand, target: "active")
        #expect(try roundTrip(expand) == expand)
        let created = ControlRequest(cmd: .workspaceNew, args: ControlArgs(name: "api", collapsed: true))
        #expect(try roundTrip(created).args?.collapsed == true)
    }
    ...
    @Test func themeSetSyncRequestRoundTrips() throws {
        let request = ControlRequest(cmd: .themeSet, args: ControlArgs(light: "Builtin Light", dark: "rook"))
        let decoded = try roundTrip(request)
        #expect(decoded.cmd == .themeSet)
        #expect(decoded.args?.light == "Builtin Light")
        #expect(decoded.args?.dark == "rook")
    ...
    @Test func themeListResponseCarriesSyncState() throws {
        let response = ControlResponse(ok: true, result: ControlResult(
            theme: "rook", themes: ["rook", "Builtin Light"], sync: true, light: "Builtin Light", dark: "rook"))
        let decoded = try roundTrip(response)
        #expect(decoded == response)
        #expect(decoded.result?.sync == true)
        #expect(decoded.result?.light == "Builtin Light")
        #expect(decoded.result?.dark == "rook")
    }
