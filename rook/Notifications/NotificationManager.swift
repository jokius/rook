...
final class NotificationManager: NSObject, @preconcurrency UNUserNotificationCenterDelegate {
    ...
    /// legacy `NSApp.dockTile.badgeLabel` is silently suppressed for rook without it.
    func start() {
    ...
    /// Present banners even while rook is the active app (the focused-pane case is dropped before
