...
struct EventCommandsTests {
    ...
    @Test func streamStateBootstrapsAndAdvancesCursor() throws {
        ...
        // an EMPTY page still advances the cursor, so a quiet stretch is never re-scanned.
    ...
    @Test func pollBacksOffOnlyOnEmptyPagesAndWritesOneBareJSONObject() throws {
        let event = ControlEvent(seq: 6, ts: 10, kind: .status,
                                 payload: ControlEventPayload(name: "api", status: "active"))
        var responses = [
        ...
        #expect(sleeps == [0.25]) // only the empty page slept
        #expect(lines.count == 1)
        #expect(!lines[0].contains("\n"))
        #expect(try JSONDecoder().decode(ControlEvent.self, from: Data(lines[0].utf8)) == event)
    }

    @Test func pollStopsImmediatelyOnTransportServerAndOutputFailures() throws {
        ...
        #expect(throws: EventStreamError.self) {
            try command.poll(state: &transportState, dependencies: EventStreamDependencies(
                send: { _ in throw EventStreamError("transport failed") }, sleep: { _ in }, writeLine: { _ in }
            ))
        }

        // a loud cursor failure must END the loop — silently rebaselining would hide the gap.
        var serverState = command.makeState()
        #expect(throws: EventStreamError.self) {
            try command.poll(state: &serverState, dependencies: EventStreamDependencies(
                send: { _ in ControlResponse(ok: false, error: ControlEventReadError.cursorExpired.rawValue) },
        ...
        #expect(throws: EventStreamError.self) {
            try command.poll(state: &outputState, dependencies: EventStreamDependencies(
                send: { _ in
                    ControlResponse(ok: true, result: ControlResult(events: ControlEventBatch(
                        run: run, next: 1, items: [ControlEvent(seq: 1, ts: 0, kind: .treeChanged)])))
                },
