...
/// (parse, shell-detection, group selection, dash-stripping) to the host-free `CommandRestore`/`ForegroundGroup`.
///
...
enum ForegroundProcess {
    ...
    /// pipeline that outlives its `sudo` does report — see `ForegroundGroup.descentCandidates`.
    @MainActor
    static func running(for view: GhosttySurfaceView, shellBasename: String?) -> [String]? {
        ...
        for pid in ForegroundGroup.descentCandidates(pgid: pgid, members: processGroup(pgid: pgid)) {
    ...
    private static func usable(_ argv: [String], shellBasename: String?) -> [String]? {
        ...
        return ForegroundGroup.stripLoginDash(argv)
    }
    ...
    ///
    /// A group that GROWS past the sizing call's slop returns ENOMEM, and macOS reports `oldlen` 0 there,
    /// so nothing can be salvaged from the buffer however full it is. Retry with a widening margin
    /// instead, rather than reporting a busy pane as idle.
    private static func processGroup(pgid: pid_t) -> [ForegroundGroup.Member] {
