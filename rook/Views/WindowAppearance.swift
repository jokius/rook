...
enum WindowAppearance {
    ...
    /// The sidebar's tagged scroll view (`rook-sidebar-scroll`), searched from the window's root view.
    private static func sidebarScroll(in window: NSWindow) -> NSScrollView? {
        guard let contentView = window.contentView else { return nil }
        var root: NSView = contentView
        while let parent = root.superview { root = parent }
        return root.firstDescendant(withIdentifier: "rook-sidebar-scroll") as? NSScrollView
    }
