...
final class PaletteUITests: XCTestCase {
    ...
    func testThemePickerCommitsOnEnterAndRevertsOnEsc() throws {
        ...
        // observable contract here). "Dracula" differs from the seeded rook default, so it proves a change.
        openThemePicker()
    ...
    func testThemePickerAutoFocusesFieldFromActionPaletteLauncher() throws {
        ...
        app.typeText("rook")
        app.typeKey(.return, modifierFlags: [])
        XCTAssertTrue(poll { self.settingsTheme() == "rook" },
                      "the auto-focused picker should filter on the typed text and commit that theme")
    }

    func testFreshLaunchAppliesRookDefaultThemeWithoutAnyChange() throws {
        // a fresh install must apply the seeded rook default at LAUNCH, not only after a settings change
        ...
        XCTAssertTrue(poll { self.appliedGhosttyTheme()?.contains("rook") == true },
                      "a fresh launch should write the rook default into the live ghostty config")
    }
