...
/// The split divider's cursor writer, which nothing else covers: `NSCursor` is process-global, so its gates
/// are only observable by driving `mouseMoved` against a real `NSSplitView` and reading `NSCursor.current`.
/// Also pins the tracking-area handback — `NSTrackingArea` does not retain its owner, so a probe that leaves
/// the window without handing its area back leaves the split messaging a freed view on the next mouse move.
@MainActor
