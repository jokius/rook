...
struct Config: ParsableCommand {
    ...
    struct Reload: RequestCommand {
        static let configuration = CommandConfiguration(abstract: "Re-read and apply the rook-scoped ghostty.conf (prints the diagnostic count).")
...
struct Quick: ParsableCommand {
    ...
    /// `rookctl quick [show|hide|toggle]` — the default, so the bare verb keeps working. Shows/hides the
    ...
    /// `rookctl quick type TEXT` — inject literal keystrokes into the frontmost window's quick terminal
    ...
    /// `rookctl quick text` — print the frontmost window's quick-terminal buffer as plain text (the
...
/// `rookctl dashboard <ids…> [--font-size N | --auto-size] [--window W]` opens a view-only grid of the
/// named sessions (max 9); `rookctl dashboard --mru [--font-size N | --auto-size] [--window W]` opens one
/// of the window's most-recently-used sessions (up to 9) instead of naming ids; `rookctl dashboard --close
/// [--window W]` closes the open one. An id may carry a `:left`/`:right` pane suffix to place ONE pane of a
/// split session. The positional ids map to `ControlArgs.targets`; the dispatcher validates the flags and
/// the pane grammar, while the 9-cell cap, the session+pane dedup, and the drop report are app-side, since
/// expanding an id into cells needs the store. The CLI re-checks the flag combinations `validate()`-style so
/// a bad invocation is a clean usage error without a socket round-trip (the dispatcher enforces the same
/// rules server-side).
...
struct Sidebar: ParsableCommand {
    ...
    /// `rookctl sidebar [show|hide|toggle]` — the default, so the bare verb keeps working. Toggles the
    ...
    /// `rookctl sidebar mode [tree|flagged|toggle]` — flips the frontmost window's sidebar view between
    ...
    /// `rookctl sidebar expand [--window W]` — expand every workspace in a window's sidebar tree
    ...
    /// `rookctl sidebar collapse [--window W]` — collapse every workspace except the active one (it
