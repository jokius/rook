...
/// Host-free (`rookCore`, Foundation + Observation only) and `@MainActor`, mirroring
...
public final class DashboardController {
    ...
    /// exists even though its shell is alive. Rewriting it to `.primary` keeps a grid built from `<id>:right`
    /// watching the same shell; without this, reconcile prunes the cell and the watched program silently
    ...
    /// promotion happened. Collapses into an existing `.primary` cell rather than duplicating it — a bare id
    /// contributes both — and carries the highlight across. A no-op without a `.split` member.
