...
extension rookApp {
    ...
    var appCommands: some Commands {
            // App menu: replace the default "About Rook" with one that opens the standard
            // panel enriched with a clickable repo link and, on release builds, the build commit.
            CommandGroup(replacing: .appInfo) {
                Button("About Rook") { showAboutPanel() }
            }
            // Edit: drop SwiftUI's stock Undo/Redo. rook registers no NSUndoManager, so they are dead for
            ...
            // File: replace the default "New" group with all of rook's creation/management actions,
            ...
            CommandGroup(replacing: .newItem) {
                ...
                Button("Close Session") {
                    ...
                    // the deck behind the panel and takes a session with it. Same predicate as
                    // `CustomCommandRunner`'s no-surface gate. A nil key window keeps the deck behavior.
                    // Gated on close_session still holding ⌘W: rebound off it, the chord belongs to whoever
                    // claims it — the stock item takes it back when NO built-in does (`anyBuiltinOwns`) — so
                    // the new chord means only what it says.
                ...
                // open the rook-scoped ghostty.conf in $EDITOR in a 95% overlay; it reloads the config on
            ...
            CommandGroup(replacing: .help) {
                Button("Developer Documentation…") {
                    if let url = URL(string: "https://github.com/jokius/rook#scripting-rook") {
    ...
    /// Opens the standard About panel, enriched with a clickable rook.app link and — on release
