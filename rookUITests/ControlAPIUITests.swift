...
/// `ROOK_STATE_DIR` (which also locates the unix socket at `<stateDir>/rook.sock`), speak the socket
...
final class ControlAPIUITests: ControlAPITestCase {
    ...
    func testSessionNewWithCommandRunsAsProcess() throws {
        let marker = NSTemporaryDirectory() + "rook-cmd-\(UUID().uuidString).txt"
    ...
    func testSessionPasteWithImageInsertsTempPNGPath() throws {
        ...
        for _ in 0..<8 where pasted == nil {
            ...
            for _ in 0..<8 {
                ...
                if text.contains("rook-paste-"), text.contains(".png") {
                    pasted = text
                    break
                }
    ...
    func testSessionSearchSecondNeedleReportsFreshCount() throws {
        let rare = "rookRARE"     // appears few times
        let common = "rookCOMMON" // appears many more times
    ...
    // returned id (and the persisted selectedSessionID) track it. wrap is covered by the rookCore tests.
    ...
    // ROOK_STATE_DIR-isolated), so a count==0 assert would be flaky on a host with its own config.
    ...
    func testThemeListAndSet() throws {
        // list: a non-empty set of bundled themes, including the repo's own "rook" theme. a fresh
        // install seeds the rook theme as the default, so it is the current theme.
        ...
        XCTAssertTrue(themes.contains("rook"), "the bundled theme set should include the repo's rook theme")
        XCTAssertEqual(listResult["theme"] as? String, "rook", "a fresh install defaults to the rook theme")
