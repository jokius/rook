...
- **For an agent, `active` is the USER's GUI-selected session, not yours.** Your shell is
  `$ROOK_SESSION_ID`; the user is usually on a different session while you work. Pass
  `--target "$ROOK_SESSION_ID"` on any session-scoped command (`overlay open`, `scratch`, `type`,
...
`scratch` (scratch shown), `fileTreeVisible` (whether the file-tree panel
...
- `session resize (--split-ratio R | --grow-left D | --grow-right D) [--target] [--window W]` — move the
  ...
  key by mapping a `command "rookctl session resize …"` custom action). Provide exactly one form:
...
- `session overlay open <command> [--cwd DIR] [--wait] [--block] [--size-percent N] [--background-color #rrggbb] [--follow] [--target] [--window W]`
  ...
  to close). `--block` waits for the command to exit and makes rookctl exit with the command's status
  ...
  `active`, so an automated caller should pass `--target "$ROOK_SESSION_ID"` — otherwise a (usually
...
- `session overlay resize (--size-percent N | --full) [--target] [--window W]` — resize an ALREADY-OPEN
  ...
  session id. Against a HUD a percent is accepted and re-flows its WIDTH (the panel re-flows and its
  `hud.sizePercent` reports the new value; `hud.heightPercent` does not move, the text wrapping at a fixed
  60 columns rather than at the panel) but `--full` is refused with
  `a hud is always floating: pass --size-percent, not --full` — full size would cover the session the
  message is about. The resize rewrites the body header itself, so the panel re-centres on its new grid
  within a tick — no `session hud update` is needed to correct the placement.
- `session overlay close [--target] [--window W]` — close (destroy) the overlay. It also takes a HUD down,
...
- `session overlay result [--target] [--window W]` — returns `result.exitCode` once the overlay has
  closed. Errors `still running` while up, `no result` if none ran. A HUD runs the app's own painter, not a
  caller's program, so there is no status to report and it errors
  `no overlay result: the slot holds a hud`.
...
- `session hud update <message> [--detail T] [--spinner] [--spinner-style S] [--position P] [--size-percent N] [--target] [--window W]`
  ...
  with; the frames ride the message file, so the look changes on the next tick with no re-spawn. Same
  required message and same rejections as `open`. There is no `--background-color`: the surface reads that
  once at creation, so only a fresh `session hud` can change it, and `tree` keeps reporting the creation
...
`rookctl surface zoom [show|hide|toggle] [--target SURFACE_ID|active|quick] [--window W]` — zoom one
...
`rookctl tree --json` at `.result.tree.workspaces[].sessions[].surfaces[].id`, for example
...
`rookctl dashboard <ids…> [--font-size N | --auto-size] [--window W]` opens a per-window, view-only
grid of the named sessions' live panes; `rookctl dashboard --mru [--font-size N | --auto-size]
[--window W]` opens the window's most-recently-used sessions instead of naming ids; `rookctl dashboard
...
ragged last row), Enter (or a double-click on a cell) jumps into the highlighted session AND focuses that
exact pane (selecting the session, focusing the primary pane for a `:left` cell or the split pane for a
`:right` cell, then closing the dashboard), and Esc closes it (leaving the selection as it was). While the
grid is up the window chrome is stripped to a bare title bar with an exit button (the same close-and-refocus
path as Esc), and Rook's Markdown-preview and file-tree panels stay MOUNTED behind the grid but take no
input. Because a cell takes no input, a program you dashboard keeps running but you cannot type into it
from the grid — jump in with Enter first.
...
untouched. Closing a member session (or its split pane) while the grid is up prunes that cell; the
dashboard closes itself when the last member is gone.
...
`rookctl quick type TEXT` (or `--stdin`) — inject `TEXT` as literal keystrokes into the frontmost
...
`rookctl quick text [--all] [--lines N]` — print the frontmost window's quick-terminal buffer as
...
`rookctl sidebar [show|hide|toggle]` — show/hide the frontmost window's workspace/session sidebar
...
`rookctl sidebar mode [tree|flagged|toggle]` — flip the frontmost window's sidebar VIEW between the
...
`rookctl sidebar expand [--window W]` — expand every workspace row in a window's sidebar tree.
...
`rookctl sidebar collapse [--window W]` — collapse every workspace EXCEPT the active one (the
...
`rookctl notify <body> [--title T] [--target] [--window W]` — post a macOS desktop notification
...
`rookctl font inc|dec|reset [--pane left|right|scratch] [--target] [--window W]` — increase / decrease /
...
`rookctl config reload` - re-read and apply the ghostty config; returns `result.count` = the ghostty
config-diagnostic count (0 = clean), counted across ALL config sources, not just the rook-scoped
...
`rookctl theme set [name]` — set and persist the terminal theme app-wide (the same change as Settings
▸ Appearance), per slot:
...
`rookctl restore clear` — clear every session's saved CAPTURED foreground command and persist, so the
...
`no overlay` / `still running` / `no result` (overlay),
...
`invalid position: <value> (top|center|bottom)` (session hud over the raw socket; the `rookctl` CLI
